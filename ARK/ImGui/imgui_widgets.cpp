...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(11, 11, 11, 255), 3.f);

    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(accent_color[0], accent_color[1], accent_color[2], it_anim->second.active_rect_alpha), 3.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = ImRect(bb_frame.Min - ImVec2(1, -2), bb_frame.Max - ImVec2(1, -3));
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor(accent_color[0], accent_color[1], accent_color[2], 255), style.ScrollbarRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    //int active_outline_anim = animate(label, "active_outline_anim", 20, 3, STATIC);
    //int active_text_anim = animate(label, "active_text_anim", 100, 8, STATIC);
    //int active_rect_alpha = animate(label, "active_outline_anim", 25, 1, STATIC);

    window->DrawList->AddRectFilled(rect.Max - ImVec2(15, 15), rect.Max, ImColor(11, 11, 11, 255), 2.f);

    window->DrawList->AddRectFilled(rect.Max - ImVec2(15, 15), rect.Max, ImColor(accent_color[0], accent_color[1], accent_color[2], it_anim->second.active_rect_alpha), 2.f);

    RenderCheckMark(window->DrawList, rect.Max - ImVec2(12, 12), ImColor(255, 255, 255, it_anim->second.active_rect_alpha), 9.f);

    window->DrawList->AddRect(rect.Max - ImVec2(15, 15), rect.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 2.f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (it_anim->second.active_circle_anim > 1)
        it_anim->second.active_circle_anim = 1;
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(11, 11, 11, 255), 3.f);

    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 3.f);
    ...
    window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 13), 4.f + it_anim->second.active_circle_anim, ImColor(accent_color[0], accent_color[1], accent_color[2], 255));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), grab_bb.Max - ImVec2(10.f, 5.f), ImColor(accent_color[0], accent_color[1], accent_color[2], 255));

    window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(5.f, 6.f), 6.f + it_anim->second.circle_anim, ImColor(accent_color[0], accent_color[1], accent_color[2], 255), 30.f);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton(label, col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(g.LastItemData.Rect.GetBL() + ImVec2(-80.0f, style.ItemSpacing.y));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = GetFrameHeight();
    const float size = ImGui::GetWindowWidth() - 20;
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos +  ImVec2(size, 20));
    const ImRect clickable(rect.Max - ImVec2(20, 20), rect.Max);
    ItemSize(rect, g.Style.FramePadding.y);
    if (!ItemAdd(clickable, id))
        return false;
    ...
    window->DrawList->AddText(rect.Min + ImVec2(0, 4), ImColor(255, 255, 255, 255), desc_id);

    if (col_rgb.w < 1.0f)
        RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max, GetColorU32(col_rgb), 5.f, ImVec2(0.0f, 0.0f), 3.f);
    else
        window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(col_rgb), 3.f);
...
bool ImGui::selection(const char* label, ImTextureID user_texture_id, bool selected)
{
    ...
    static std::map <ImGuiID, selection_anim> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end())
    {
        anim.insert({ id, {0, 0} });
...
bool ImGui::group(const char* label, bool selected)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map <ImGuiID, group_anim> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end())
    {
